...
function getTotalTips (billArray){
    return calculateTips(billArray[0])+calculateTips(billArray[1])+calculateTips(billArray[2])+calculateTips(billArray[3]);
}
console.log("Your total tip amount is $" + getTotalTips(billArr));

function budgetDust(arr){
    let totalBill =  arr[0]+ arr[1]+ arr[3]+ arr[2];
    let lastBudget = totalBudget - (totalBill + getTotalTips(arr));
